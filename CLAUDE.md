...
- Kein Build-Schritt, kein `npm install` für die App selbst — Vanilla JS (ES-Module) + Tailwind Play CDN + Dexie, beide per CDN-`<script>`-Tag eingebunden. Ausnahme: `tests/` (s. unten) hat ein eigenes, rein Dev-seitiges `package.json` für automatisierte Datenlogik-Tests — betrifft nur lokales Testen, nicht die Auslieferung der App
...
- Löschen/Entfernen von irgendetwas (Übungen, Routinen, Profil): immer Bestätigungsdialog (`confirm()`). **Ausnahmen:** Sätze — deren Löschen läuft (Übungs-Detailseite, Abschnitt 12) bewusst ohne Bestätigungsdialog, auf ausdrücklichen Nutzer-Wunsch, s. [ADR 0010](docs/decisions/0010-uebungs-detailseite.md). Ebenso das Entfernen einer einzelnen, noch unbegonnenen Übung aus dem Tages-Roster (Workout-Tab, "⋮"-Menü, `removeExerciseFromWorkout`) — betrifft nie Trainingsdaten, s. [ADR 0016](docs/decisions/0016-swipe-to-delete-roster.md)/[ADR 0017](docs/decisions/0017-kontextmenue-redesign-referenzbild.md). Kaskaden-Regeln für Trainingsdaten stehen in [ADR 0004](docs/decisions/0004-loesch-kaskaden.md) und [ADR 0007](docs/decisions/0007-workout-tab-tagesbasiertes-modell.md) — nicht ohne Rücksprache ändern. Kein Weg, ein ganzes Tages-Workout zu löschen (bewusst nicht gebaut, s. ADR 0007) — nur einzelne Sätze, einzelne noch unbegonnene Übungen (`removeExerciseFromWorkout`) oder das Entfernen einer Routine
- **Automatisierte Tests** für die heiklen Datenfunktionen in `js/db.js` (Löschkaskaden, Routine-Wechsel, Tages-Workout-Eindeutigkeit) liegen in `tests/` — `npm test` (Node's eingebauter Test-Runner, `fake-indexeddb` simuliert IndexedDB, kein zusätzliches Test-Framework). Vor Änderungen an diesen Funktionen laufen lassen; bei neuen heiklen Datenfunktionen einen passenden Test ergänzen. Deckt bewusst nur Datenlogik ab, keine UI — dafür weiterhin manuell im Browser verifizieren (s. nächster Punkt)
