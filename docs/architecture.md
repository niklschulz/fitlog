...
Dieselbe Grundmechanik (Pointer Events, direkte `transform`-Manipulation während der Geste statt über `paint()`) nutzt seit [ADR 0016](decisions/0016-swipe-to-delete-roster.md) auch die Swipe-to-Delete-Geste auf den Roster-Karten im Workout-Tab (`wireExerciseRosterSwipe()` in `workout.js`) - unabhängig von `js/sheet.js` implementiert (andere Achse, anderer Aufrufer-Vertrag), aber demselben Prinzip folgend. Dort auch die Begründung für eine Falle mit `setPointerCapture()`, die den nativen `click` auf verschachtelten Kind-Elementen unzuverlässig macht - relevant für jede künftige Pointer-Event-Geste auf einem Element mit interaktiven Kindern.
