...
**Ausnahme**: Bottom-Sheets (großer Kalender, [ADR 0009](decisions/0009-grosser-kalender-vollstaendiges-rendern.md); Übungs-Sheet + die beiden gestapelten Übungs-Detail- und Neue-Übung-Sheets, [ADR 0011](decisions/0011-uebungs-sheet-bottom-sheet-extraktion.md) bzw. [ADR 0014](decisions/0014-neue-uebung-sheet-gestapelt.md), alle in `workout.js`) manipulieren das DOM während des Drag-to-Dismiss-Gestus am Ziehgriff bewusst direkt statt über `paint()` (muss 1:1 dem Finger folgen). Die geteilte Mechanik dafür (Body-Scroll-Sperre, Nav-z-index, Drag-Pointer-Events) steckt in `js/sheet.js` (ADR 0011) - State, Öffnen/Schließen-Zeitpunkt und Render-Inhalt jedes einzelnen Sheets bleiben beim jeweiligen View-Modul. Historisch gab es beim großen Kalender zusätzlich Lazy-Nachladen weiterer Monate beim Scrollen ([ADR 0008](decisions/0008-grosser-kalender-lazy-loading.md)), das spürbares Ruckeln verursachte und durch vollständiges Vorab-Rendern ersetzt wurde.

Dieselbe Grundmechanik (Pointer Events, direkte `transform`-Manipulation während der Geste statt über `paint()`) nutzte zwischenzeitlich auch eine Swipe-to-Delete-Geste auf den Roster-Karten im Workout-Tab ([ADR 0016](decisions/0016-swipe-to-delete-roster.md)) - wieder entfernt, s. [ADR 0017](decisions/0017-kontextmenue-redesign-referenzbild.md). Die dabei gefundene Falle mit `setPointerCapture()`, die den nativen `click` auf verschachtelten Kind-Elementen unzuverlässig macht, bleibt als Lernpunkt in ADR 0016 dokumentiert - relevant für jede künftige Pointer-Event-Geste auf einem Element mit interaktiven Kindern, auch ohne aktuellen Anwendungsfall im Code.

Kalender-Sheet und Übungs-Sheet (die beiden nicht-gestapelten, "untersten" Sheets) rendern ihren Inhalt beim Öffnen/Schließen weiterhin einmalig über den normalen `paint()`-Zyklus, da dabei nichts anderes im DOM steht, was dadurch versehentlich mit angefasst würde. Die beiden gestapelten Sheets (Übungs-Detail-Sheet, Neue-Übung-Sheet) öffnen/schließen dagegen bewusst NICHT über `paint()`, sondern per direkter DOM-Manipulation (`insertAdjacentHTML('beforeend', …)` beim Öffnen; `classList.add('closing')` + `setTimeout` + `.remove()` beim Schließen) und tauchen dafür auch nicht mehr im deklarativen Render-Ausdruck der obersten `paint()`-Funktion auf — Details und Begründung: [ADR 0015](decisions/0015-gestapelte-sheets-ohne-paint.md). Grund: `paint()` ersetzt `currentContainer.innerHTML` komplett und hätte damit auch das bereits offene, darunterliegende Sheet neu aufgebaut — dessen `.bottom-sheet`/`.bottom-sheet-backdrop`-CSS-Klassen tragen `animation`-Eigenschaften, die bei jeder DOM-(Neu-)Einfügung erneut abspielen, wodurch das darunterliegende Sheet sichtbar neu eingeflogen wäre, obwohl es die ganze Zeit sichtbar war. Dieselbe Grundregel wie beim Suchfeld-Teil-Repaint unten, nur eine Ebene höher: nie ein ganzes bereits offenes/animiertes Sheet unnötig neu aufbauen.
...
- **iOS-Update-Verhalten (Service Worker)**: Eine als Home-Bildschirm-App (standalone) geöffnete PWA prüft von sich aus deutlich seltener auf Service-Worker-Updates als ein normaler Safari-Tab — ein einfaches Neuöffnen reicht auf iOS oft nicht. Gegenmaßnahme in `app.js`: `registration.update()` explizit bei jedem App-Start und bei `visibilitychange` (zu "visible") aufrufen, plus ein `controllerchange`-Listener, sobald ein neuer Service Worker die Kontrolle übernimmt (dank `skipWaiting`/`clients.claim()` in `sw.js` passiert das automatisch, sobald ein Update gefunden wurde) — damit ist die neue Version zeitnah sichtbar statt erst nach mehrfachem manuellem Neustart. Der Reload selbst läuft aber NICHT bedingungslos: Ist `document.visibilityState` zu diesem Zeitpunkt `"visible"` (App gerade aktiv in Benutzung — offenes Sheet, halb ausgefülltes Formular), wird er auf den nächsten Wechsel in den Hintergrund zurückgestellt (`pendingReloadAfterUpdate`-Flag) statt sofort auszuführen; ist die App bereits unsichtbar, läuft er sofort, da das dann niemanden stört. Ursprünglich bedingungslos, per Nutzer-Bugreport korrigiert (s. design-system.md, Einundneunzigste Iteration): ein zufällig mit einem App-Update zusammenfallendes Öffnen eines Sheets (z. B. "Profil verknüpfen") wirkte dadurch wie ein spontanes Schließen des Sheets samt Rücksprung zum Start-Tab. Mit einem simulierten Update lokal verifiziert (Reload löst tatsächlich aus, alte Caches werden korrekt ersetzt).
