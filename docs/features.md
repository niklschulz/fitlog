...
Pro Tag: **Routine wählen** (Popup-Liste aller Routinen) legt deren Übungen als Roster für diesen Tag an. "Alle Routinen anzeigen" am Ende der Popup-Liste öffnet ein eigenes **Routinen-Sheet** (Bottom-Sheet wie der Kalender) mit je einer Karte pro Routine (Name + Übungs-Anzahl, gleicher Aufbau wie eine Roster-Karte). Ein "⋮"-Button pro Karte öffnet ein Kontextmenü mit **Bearbeiten** und **Löschen** (mit Bestätigungsdialog). Sowohl der "+"-Button oben rechts (Neuanlage) als auch "Bearbeiten" öffnen denselben zweiten Inhalts-Modus desselben Routinen-Sheets (kein eigenes Sheet, s. design-system.md, Hundertsechste Iteration): ein Namensfeld sowie darunter "Übungen hinzufügen", das das bestehende **Übungs-Sheet** (s. unten) gestapelt darüber öffnet — Auswahl dort landet als Liste im Entwurf, jede Zeile mit einem "⋮"-Kontextmenü (einziger Eintrag "Entfernen", ohne Bestätigungsdialog, da nur der Entwurf betroffen ist). Reiner Entwurf im State: Weder die Routine noch ihre Übungen landen in der DB, bevor der Speichern-Haken oben rechts (deaktiviert ohne Namen) gedrückt wird; "✕" oben links verwirft den Entwurf und kehrt zur Liste zurück. **Umsortieren** per Gedrückthalten und Verschieben einer Übungs-Zeile; die Reihenfolge im Entwurf wird beim Speichern übernommen. Auswählen einer Routine für den Tag läuft weiterhin ausschließlich über die kleine Popup-Liste darüber, nicht über dieses Sheet — schrittweiser Ausbau des Routinen-Bereichs, weitere Schritte folgen. **Wechseln**/**Entfernen** einer Routine räumt noch nicht begonnene, routinen-stammende Übungen auf, behält aber bereits begonnene (mit erfassten Sätzen) unabhängig davon, ob sie in einer neuen Routine enthalten sind — Details und Beispiel: [ADR 0007](decisions/0007-workout-tab-tagesbasiertes-modell.md). Ist die bearbeitete Routine bereits für den aktuell im Workout-Tab gewählten Tag ausgewählt, wird dessen Roster beim Speichern der Routine automatisch nach derselben Regel nachsynchronisiert (nicht rückwirkend für andere Tage). **"+ Übung hinzufügen"** öffnet das **Übungs-Sheet** (gleiches Bottom-Sheet-Muster wie der Kalender, gestapelt/wiederverwendbar gemacht, s. [ADR 0011](decisions/0011-uebungs-sheet-bottom-sheet-extraktion.md)): Suchfeld (Live-Filter beim Tippen) und darunter ein Muskelgruppen-Filter (Dropdown-Pill wie die Routine-Auswahl, matcht primäre *oder* sekundäre Muskelgruppe einer Übung, s. [ADR 0013](decisions/0013-uebung-muskel-verknuepfung.md)), darunter die Liste aller angelegten Übungen. Jede Zeile zeigt Name plus primären Muskel als Untertitel, ein eckiges Auswahl-Kästchen rechts sammelt eine Mehrfachauswahl (heute schon vorhandene Übungen bleiben sichtbar, aber mit deaktiviertem Häkchen-Badge markiert), eine Leiste unten ("Hinzufügen") committet die gesamte Auswahl gesammelt. "+"-Button oben rechts öffnet das **Neue-Übung-Sheet** — ein weiteres, über dem Übungs-Sheet gestapeltes Sheet (Schließen führt nur zum Übungs-Sheet zurück, nicht zum Workout-Tab, s. [ADR 0014](decisions/0014-neue-uebung-sheet-gestapelt.md)): Namensfeld plus Chip-Auswahl für primären (Einzelauswahl) und sekundäre (Mehrfachauswahl) Muskeln aus derselben festen Acht-Muskelgruppen-Liste. Neu erstellte Übung landet automatisch in der Mehrfachauswahl des Übungs-Sheets. Tap auf den Übungsnamen selbst öffnet statt der Auswahl ein weiteres, ebenfalls gestapeltes **Übungs-Detail-Sheet** — Kopfzeile (✕, Übungsname, roter Löschen-Button mit Bestätigungsdialog) ist fertig, der eigentliche Inhalt ist noch ein Platzhalter ("Weitere Details folgen."). Löschen (`deleteExercise()`, s. Lösch-Verhalten unten) aktualisiert das Übungs-Sheet dahinter sofort. Diese manuell hinzugefügten Übungen bleiben von Routine-Wechsel/-Entfernen unberührt (`sourceRoutineId: null`).

Jede Roster-Zeile zeigt unter dem Übungstitel bereits die vollständige, nummerierte Satz-Liste (verbunden durch eine dünne vertikale Linie zwischen den Nummern-Kreisen, je Zeile "35 kg" / "10 Reps" getrennt) — ohne die Detailseite öffnen zu müssen. Tap auf Titel/Satz-Liste öffnet die **Übungs-Detailseite** (`js/views/workout-exercise-detail.js`, Abschnitt 12) — eigene Unterseite mit Zurück-Pfeil (kreisförmiger Glass-Button) zur Tagesübersicht und drei Reitern (Segmented Control): aktueller Tag, Verlauf, Statistik (Platzhalter). Sobald der erste Satz für eine Übung erfasst wird, rückt sie im Roster automatisch nach vorne (dynamische Sortierung nach Bearbeitungszeitpunkt statt Routinen-Reihenfolge). Noch unbegonnene Übungen (keine Sätze erfasst) lassen sich zusätzlich aus dem heutigen Roster entfernen (`removeExerciseFromWorkout()`, die Übung selbst bleibt erhalten): ein "⋮"-Button vertikal auf Höhe des Titels öffnet ein kleines Kontextmenü ("Übung entfernen", nach einem iOS-Referenzbild gestaltet — überlagert die Karte direkt, `rounded-sheet`-Form, Liquid-Glass-Look der Bottom-Nav samt deren Standard-Schatten, s. [ADR 0017](decisions/0017-kontextmenue-redesign-referenzbild.md)), ohne Bestätigungsdialog (bewusste Ausnahme, s. [ADR 0016](decisions/0016-swipe-to-delete-roster.md)). Bereits begonnene Übungen (mit Sätzen) bekommen diesen Button nicht, dieselbe Schutzregel wie bei jeder anderen workoutExercises-Kaskade (s. [ADR 0007](decisions/0007-workout-tab-tagesbasiertes-modell.md)) — nur "Übung löschen" (Übungs-Detail-Sheet, komplett aus der Datenbank) oder das Entfernen der gesamten Routine wirken auch auf begonnene Übungen.

#### Übungs-Detailseite — Reiter "aktueller Tag"
Gewicht (kg, 1-kg-Stepper, direkte Dezimaleingabe im Feld möglich) und Wiederholungen (Feld-Label "Reps", 1er-Stepper, nur ganze Zahlen) über Minus-/Plus-Buttons oder direkte Eingabe. Vorbelegung beim Öffnen mit dem zuletzt erfassten Wert dieser Übung, egal aus welchem Workout-Tag (**Progressive-Overload-Hilfe**). Tap auf einen bereits ausgefüllten Satz lädt dessen Werte zur Bearbeitung ("Ändern"-Button, "Löschen" wird aktiv); ohne Auswahl bzw. bei Tap auf eine noch leere Platzhalter-Zeile zeigt das Formular die Vorbelegung, Button heißt "Speichern", "Löschen" bleibt deaktiviert. Anzahl der standardmäßig gezeigten leeren Platzhalter-Zeilen kommt aus `getSettings().defaultSetCount` (Default 3, s. "Einstellungen" im Profil-Tab unten) statt fest im Rendering verankert zu sein. Stammt die Übung aus einer Routine, erscheint darunter "ROUTINE: `<Name>`".
...
### Statistik (`js/views/statistics.js`)
Ersetzt seit [ADR 0018](decisions/0018-uebungen-tab-entfernt.md) den bisherigen eigenständigen Übungen-Tab an derselben Position in der Bottom-Nav. Übungen anlegen/bearbeiten/löschen läuft seitdem ausschließlich über das Workout-Tab (Neue-Übung-Sheet bzw. Übungs-Detail-Sheet, s. oben) und perspektivisch über die noch zu bauende Übungs-Verwaltung innerhalb der Routinen (s. "Vorgemerkte, noch nicht spezifizierte Erweiterungen"). Kopfzeile `<h1 class="text-screen-title">Statistik</h1>` nach demselben Muster wie jeder andere Haupt-Tab, darunter eine Reiter-Zeile (Segmented Control, s. design-system.md) mit den Reitern **"Übersicht"** und **"Übungen"**. Nicht zu verwechseln mit dem gleichnamigen, ebenfalls noch leeren "Statistik"-Reiter auf der Übungs-Detailseite (Segmented Control, s. oben) — zwei unabhängige Bereiche, die künftig unterschiedliche Inhalte bekommen (dieser Tab übergreifend, der Reiter je Übung).

#### Reiter "Übersicht" — Workouts pro Woche
Erstes echtes Feature (bislang war der Tab reiner Platzhalter). Zähler-Zeile (`text-kpi`) mit **Diese Woche** (als Ziel-Fraktion, z. B. "1/5"), **Dieser Monat**, **Vorheriger Monat**, darunter ein Balkendiagramm der letzten `HISTORY_WEEKS` (Default 8) Wochen, aktuelle Woche farblich hervorgehoben. Zählt bewusst **Tage mit mindestens einem erfassten Satz** (`getTrainedDates()` in `js/db.js`), nicht rohe `workouts`-Datenbankzeilen — letztere entstehen bereits beim bloßen Wählen einer Routine oder Hinzufügen einer Übung zum Tages-Roster, auch ganz ohne einen einzigen geloggten Satz (s. `getOrCreateWorkoutForDate`). Dieselbe "dokumentiert"-Definition wie der grüne Punkt im kleinen Kalender des Workout-Tabs. Wochenziel über `getSettings().weeklyGoal` (Default 5, s. "Einstellungen" im Profil-Tab unten). Verlaufsfenster zeigt immer genau `HISTORY_WEEKS` Wochen (aktuelle + vorherige), unabhängig davon, wann das erste Training erfasst wurde; Wochen ohne Training erscheinen als Nullbalken. X-Achsen-Labels zeigen die ISO-Kalenderwoche ("KW 37"), nicht das Datum. Vollständig offline aus IndexedDB berechnet, lokale Gerätezeitzone (kein Server-Call, kein Zeitzonen-Hardcoding). Der "Weighted volume"-Chart aus der Referenz-App ist nicht Teil dieses Features.

#### Reiter "Übungen"
Reiner Platzhalter ohne Inhalt/Funktion.

Routinen werden ausschließlich über das **Routinen-Sheet** im Workout-Tab verwaltet (s. dort, [ADR 0019](decisions/0019-routinen-tab-entfernt.md)) — es gibt keinen eigenständigen Routinen-Tab mehr.

### Profil (`js/views/profile.js`) — Vorbereitung für späteren Sync
Vierter Tab, ganz rechts. Zwei Zustände: **kein Profil** (nur der "Profil verknüpfen"-Button mit Sign-In-Icon, kein zusätzlicher Hinweistext) und **Profil vorhanden** (kompakte Karte "Angemeldet als" mit Username, ohne Token-Anzeige, rechts ein Stift-Button). Das Bottom-Sheet für Username/Token dient beiden Fällen: ohne Profil öffnet es sich leer mit Titel "Profil verknüpfen" und Aktions-Button "Verknüpfen"; der Stift-Button auf der Karte öffnet dasselbe Sheet mit Titel "Profil bearbeiten", Username/Token dort als reiner, nicht editierbarer Text, und grauem Aktions-Button "Profil entfernen" (mit Bestätigungsdialog, wie überall sonst in der App) statt eines eigenständigen Lösch-Buttons unter der Karte. Ein Token-Wechsel läuft weiterhin über Entfernen + erneutes Verknüpfen, jetzt aber beides aus demselben Sheet heraus. Beide Werte werden lokal über `localStorage` gespeichert (`js/profile.js`), nicht in der Dexie-Datenbank — sind reine Geräte-Konfiguration, keine Trainingsdaten. **Der eigentliche Sync findet noch nicht statt** (kein Backend vorhanden), das Tracking funktioniert unabhängig vom Profil-Status vollständig offline weiter. Hintergrund: [ADR 0006](decisions/0006-token-basierte-nutzertrennung.md).
...
- Freischwebende, pillenförmige Bottom-Tab-Navigation mit Icons: Workout, Statistik, Routinen, Profil
...
- Übungs-Verwaltung (Umbenennen, Muskel-Zuordnung ändern) innerhalb der Routinen-Verwaltung — der eigenständige Übungen-Tab dafür wurde entfernt (s. [ADR 0018](decisions/0018-uebungen-tab-entfernt.md)), `updateExercise()` in `js/db.js` bleibt dafür vorbereitet erhalten, aber noch ohne UI-Anbindung
...
- **Eigenständiger Routinen-Tab** (`js/views/routines.js`, dritter Tab in der Bottom-Nav): Voll-Seiten-Flow aus Liste, Namensformular, Editor und Einzelauswahl-Picker mit sofortigem Live-Speichern und ▲/▼-Umsortieren. Entfernt, nachdem das Routinen-Sheet im Workout-Tab alle Funktionen (Anlegen, Bearbeiten, Übungen hinzufügen, Umsortieren, Entfernen, Löschen) übernommen und um Mehrfachauswahl, spontane Übungs-Neuanlage und die Nachsynchronisierung des Tages-Workouts erweitert hatte — zwei parallele Bearbeiten-Wege mit unterschiedlichem Verhalten waren auf Dauer nicht sinnvoll, s. [ADR 0019](decisions/0019-routinen-tab-entfernt.md).
- **Eigenständiger Übungen-Tab** (`js/views/exercises.js`, vierter Tab in der Bottom-Nav): einfache Liste aller Übungen mit Anlegen/Bearbeiten/Löschen-Formular. Entfernt, da das Anlegen/Löschen von Übungen inzwischen vollständig ins Workout-Tab (Neue-Übung-Sheet, Übungs-Detail-Sheet) verlagert ist und das Bearbeiten künftig in der Routinen-Verwaltung landen soll — ein separater, nur dafür da stehender Tab wurde dadurch redundant. Ersetzt in der Bottom-Nav durch den neuen Statistik-Tab, s. [ADR 0018](decisions/0018-uebungen-tab-entfernt.md).
