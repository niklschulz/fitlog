...
Erstes echtes Feature (bislang war der Tab reiner Platzhalter). Zähler-Zeile (`text-kpi`) mit **Diese Woche** (als Ziel-Fraktion, z. B. "1/5"), **Dieser Monat**, **Vorheriger Monat**, darunter ein Balkendiagramm der letzten `HISTORY_WEEKS` (Default 8) Wochen, aktuelle Woche farblich hervorgehoben. Zählt bewusst **Tage mit mindestens einem erfassten Satz** (`getTrainedDates()` in `js/db.js`), nicht rohe `workouts`-Datenbankzeilen — letztere entstehen bereits beim bloßen Wählen einer Routine oder Hinzufügen einer Übung zum Tages-Roster, auch ganz ohne einen einzigen geloggten Satz (s. `getOrCreateWorkoutForDate`). Dieselbe "dokumentiert"-Definition wie der grüne Punkt im kleinen Kalender des Workout-Tabs. Wochenziel über `WEEKLY_GOAL`-Konstante (`js/views/statistics.js`, Default 5) — analog zu `DEFAULT_SET_COUNT` (s. oben) als Grundlage für einen künftigen gemeinsamen Einstellungsbereich, noch nicht gebaut. Verlaufsfenster zeigt immer genau `HISTORY_WEEKS` Wochen (aktuelle + vorherige), unabhängig davon, wann das erste Training erfasst wurde; Wochen ohne Training erscheinen als Nullbalken. X-Achsen-Labels zeigen die ISO-Kalenderwoche ("KW 37"), nicht das Datum. Vollständig offline aus IndexedDB berechnet, lokale Gerätezeitzone (kein Server-Call, kein Zeitzonen-Hardcoding). Der "Weighted volume"-Chart aus der Referenz-App ist nicht Teil dieses Features.
