...
Jede Roster-Zeile zeigt unter dem Übungstitel bereits die vollständige, nummerierte Satz-Liste (verbunden durch eine dünne vertikale Linie zwischen den Nummern-Kreisen, je Zeile "35 kg" / "10 Reps" getrennt) — ohne die Detailseite öffnen zu müssen. Tap auf Titel/Satz-Liste öffnet die **Übungs-Detailseite** (`js/views/workout-exercise-detail.js`, Abschnitt 12) — eigene Unterseite mit Zurück-Pfeil (kreisförmiger Glass-Button) zur Tagesübersicht und drei Reitern (Segmented Control): aktueller Tag, Verlauf, Statistik (Platzhalter). Sobald der erste Satz für eine Übung erfasst wird, rückt sie im Roster automatisch nach vorne (dynamische Sortierung nach Bearbeitungszeitpunkt statt Routinen-Reihenfolge). Noch unbegonnene Übungen (keine Sätze erfasst) lassen sich zusätzlich aus dem heutigen Roster entfernen (`removeExerciseFromWorkout()`, die Übung selbst bleibt erhalten) — auf zwei gleichwertigen Wegen: ein "⋮"-Button vertikal auf Höhe des Titels öffnet ein kleines Kontextmenü im Liquid-Glass-Look der Bottom-Nav ("Übung entfernen"), oder die Karte lässt sich klassisch nach links wischen (iOS-Lösch-Geste wie bei Mail) — ein kurzer Swipe rastet an einem roten "Entfernen"-Button ein, ein vollständiger Swipe löscht direkt. Keiner der beiden Wege zeigt einen Bestätigungsdialog (bewusste Ausnahme, s. [ADR 0016](decisions/0016-swipe-to-delete-roster.md)). Bereits begonnene Übungen (mit Sätzen) bekommen weder Button noch Swipe-Geste, dieselbe Schutzregel wie bei jeder anderen workoutExercises-Kaskade (s. [ADR 0007](decisions/0007-workout-tab-tagesbasiertes-modell.md)) — nur "Übung löschen" (Übungs-Detail-Sheet, komplett aus der Datenbank) oder das Entfernen der gesamten Routine wirken auch auf begonnene Übungen.
