...
Pro Tag: **Routine wählen** (Popup-Liste aller Routinen) legt deren Übungen als Roster für diesen Tag an. **Wechseln**/**Entfernen** einer Routine räumt noch nicht begonnene, routinen-stammende Übungen auf, behält aber bereits begonnene (mit erfassten Sätzen) unabhängig davon, ob sie in einer neuen Routine enthalten sind — Details und Beispiel: [ADR 0007](decisions/0007-workout-tab-tagesbasiertes-modell.md). **"+ Übung hinzufügen"** öffnet das **Übungs-Sheet** (gleiches Bottom-Sheet-Muster wie der Kalender, gestapelt/wiederverwendbar gemacht, s. [ADR 0011](decisions/0011-uebungs-sheet-bottom-sheet-extraktion.md)): Suchfeld (Live-Filter beim Tippen) und darunter ein Muskelgruppen-Filter (Dropdown-Pill wie die Routine-Auswahl, matcht primäre *oder* sekundäre Muskelgruppe einer Übung, s. [ADR 0013](decisions/0013-uebung-muskel-verknuepfung.md)), darunter die Liste aller angelegten Übungen. Auswahl-Kreis links je Zeile sammelt eine Mehrfachauswahl (heute schon vorhandene Übungen bleiben sichtbar, aber mit deaktiviertem Häkchen-Badge markiert), eine Leiste unten ("Hinzufügen (n)") committet die gesamte Auswahl gesammelt. "+"-Button oben rechts öffnet das **Neue-Übung-Sheet** — ein weiteres, über dem Übungs-Sheet gestapeltes Sheet (Schließen führt nur zum Übungs-Sheet zurück, nicht zum Workout-Tab, s. [ADR 0014](decisions/0014-neue-uebung-sheet-gestapelt.md)): Namensfeld plus Chip-Auswahl für primären (Einzelauswahl) und sekundäre (Mehrfachauswahl) Muskeln aus derselben festen Acht-Muskelgruppen-Liste. Neu erstellte Übung landet automatisch in der Mehrfachauswahl des Übungs-Sheets. Tap auf den Übungsnamen selbst öffnet statt der Auswahl ein weiteres, ebenfalls gestapeltes **Übungs-Detail-Sheet** — Kopfzeile (✕ + Übungsname) ist fertig, der eigentliche Inhalt ist noch ein Platzhalter ("Weitere Details folgen."), Konzept dafür (inkl. Löschen) folgt separat. Diese manuell hinzugefügten Übungen bleiben von Routine-Wechsel/-Entfernen unberührt (`sourceRoutineId: null`).
...
- Inhalt des Übungs-Detail-Sheets (geöffnet per Tap auf eine Übung im Übungs-Sheet, [ADR 0011](decisions/0011-uebungs-sheet-bottom-sheet-extraktion.md)) — aktuell nur Kopfzeile + Platzhalter-Text, inklusive der dafür vorgesehenen Löschen-Aktion
