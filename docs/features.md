...
Vierter Tab, ganz rechts. Zwei Zustände: **kein Profil** (nur der "Profil verknüpfen"-Button mit Sign-In-Icon, kein zusätzlicher Hinweistext) und **Profil vorhanden** (zeigt Username und Token an, darunter "Profil entfernen" mit Bestätigungsdialog, wie überall sonst in der App). "Profil verknüpfen" öffnet ein Bottom-Sheet (Username + Token als Pflichtfelder, unmaskierter Text, "Verknüpfen"-Button) statt eines inline eingeblendeten Formulars. Kein Bearbeiten-Modus — ein Token-Wechsel läuft über Entfernen + erneutes Verknüpfen. Beide Werte werden lokal über `localStorage` gespeichert (`js/profile.js`), nicht in der Dexie-Datenbank — sind reine Geräte-Konfiguration, keine Trainingsdaten. **Der eigentliche Sync findet noch nicht statt** (kein Backend vorhanden), das Tracking funktioniert unabhängig vom Profil-Status vollständig offline weiter. Hintergrund: [ADR 0006](decisions/0006-token-basierte-nutzertrennung.md).
