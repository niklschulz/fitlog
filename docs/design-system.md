...
**Vierte Iteration – Inhalts-Ausrichtung + Icon-Morph-Animation (2026-08-30):**
- **Randausrichtung war nur auf Spaltenebene korrekt, nicht visuell.** Die vorherige `getBoundingClientRect()`-Prüfung maß nur die (unsichtbaren) Grid-Spalten des Wochenblocks — die waren tatsächlich pixelgenau. Übersehen: der sichtbare Inhalt jeder Spalte (Wochentag-Text, Tageszahl) war per `items-center` innerhalb seiner vollen ~49px breiten Spalte zentriert, wirkte dadurch optisch eingerückt. Montag (erste Spalte) bekommt jetzt `items-start`, Sonntag (letzte Spalte) `items-end`, die übrigen fünf Tage bleiben `items-center`. Diesmal am tatsächlich sichtbaren `<span>`-Inhalt verifiziert (nicht nur am Spalten-Container) — 0px Abweichung zu Überschrift/Icon für Wochentag- **und** Tageszahl-Text
- **Chevron-zu-X-Morph-Animation** für das Dropdown-Icon: zwei 7×1.5px-Balken (`±45°` rotiert), die in Ruhestellung einen Chevron nach unten bilden (Balken bei `left: 6px`/`10px`, treffen sich unten mittig) und beim Öffnen zu `left: 8px` zusammenschieben, wo sie sich zu einem X überlagern — nur `left` wird animiert, die Rotation bleibt konstant, dadurch eine einfache, sauber interpolierbare Bewegung statt einer komplexen SVG-Pfad-Morphung. Nutzt wie das Popup `@keyframes` (nicht `transition`) mit `animation-fill-mode: forwards`, da das Icon bei jedem Paint neu ins DOM eingefügt wird; die Animationsklasse (`is-open`/`is-closing`) hängt direkt an `state.routinePickerOpen`/`-Closing` ohne zusätzliches "hat sich gerade geändert"-Flag, da während geöffnetem Picker jede Interaktion zwangsläufig zum Schließen führt (Backdrop blockiert den Rest der Seite) — es gibt also keinen Zwischenzustand, in dem ein Paint aus anderem Grund die Animation fälschlich erneut abspielen könnte. Synchron mit der Popup-Animation (200ms, gegen deren 150ms) per `getComputedStyle()` während und nach beiden Übergängen verifiziert
