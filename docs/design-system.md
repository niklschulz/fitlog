...
- **Wochenweises Einrasten statt freiem Scroll:** Der Datumsbereich wird jetzt auf ganze Mo-So-Wochen ausgerichtet (`mondayOf()`-Helper in `workout.js`), `scroll-snap-align: start` sitzt gezielt nur auf den Montags-Buttons (`i % 7 === 0`), alle anderen Tage bekommen keinen Snap-Punkt — dadurch rastet die Zeile pro voller Woche ein statt pro Einzeltag, bei weiterhin `basis-[14.2857%]` (7 Tage pro Bildschirmbreite). Der Auto-Scroll beim Laden richtet sich am Wochenanfang aus (`inline: 'start'`), nicht mehr zentriert auf den Einzeltag
