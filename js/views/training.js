...
async function renderIdle() {
  ...
  return `
    <div class="py-4 flex flex-col gap-6">
      <h1 class="text-screen-title">Training</h1>

      <button id="start-no-routine-btn" class="tap-feedback bg-accent text-base font-bold rounded-btn py-4 text-lg min-h-[44px]">
        Training starten
      </button>
      ...
      ${
        routines.length > 0
          ? `<div class="flex flex-col gap-2">
              <h2 class="text-label text-muted uppercase">Oder mit Routine starten</h2>
              <ul class="flex flex-col gap-2">
                ${routines
                  .map(
                    (r) => `
                  <li>
                    <button data-routine="${r.id}" class="start-with-routine-btn tap-feedback w-full text-left bg-surface rounded-btn px-4 py-3 min-h-[44px] text-card-title">
                      ${escapeHtml(r.name)}
                    </button>
                  </li>
                `
...
async function renderActive() {
  ...
  return `
    <div class="py-4 flex flex-col gap-4">
      <h1 class="text-screen-title">Training läuft</h1>
      ...
      <div class="flex flex-col gap-2">
        <h2 class="text-label text-muted uppercase">Erfasste Sätze (${sets.length})</h2>
        ${
          sets.length === 0
            ...
            : `<ul class="flex flex-col gap-2">
                ${sets
                  .map(
                    (s) => `
                  <li class="bg-surface rounded-btn px-4 py-2 flex items-center justify-between">
                    <span class="text-body">${escapeHtml(exerciseNameById[s.exerciseId] ?? 'Gelöschte Übung')} — ${s.weight} kg × ${s.reps}</span>
      ...
      <button id="finish-workout-btn" class="tap-feedback bg-raised text-ink font-semibold rounded-btn py-3 min-h-[44px] mt-2">
        Training beenden
      </button>
    </div>
  `;
...
function renderExercisePicker(routineExercises, otherExercises) {
  const chips = routineExercises
    .map(
      (ex) => `
      <button data-exercise="${ex.id}" class="exercise-chip tap-feedback rounded-full px-4 py-2 min-h-[44px] text-body font-medium ${
        state.selectedExerciseId === ex.id ? 'bg-accent text-base' : 'bg-surface text-ink'
      }">
        ${escapeHtml(ex.name)}
      </button>
    `
  ...
  return `
    <div class="flex flex-col gap-2">
      <div class="flex flex-wrap gap-2">
        ${chips}
        <button id="toggle-picker-btn" class="tap-feedback rounded-full px-4 py-2 min-h-[44px] text-body font-medium bg-surface text-accent">
    ...
    <div class="bg-surface rounded-card p-3 flex flex-col gap-2">
      ${
        otherExercises.length === 0
          ? `<p class="text-body text-muted py-2">Keine weiteren Übungen vorhanden.</p>`
          : `<ul class="flex flex-col gap-1 max-h-48 overflow-y-auto">
              ${otherExercises
                .map(
                  (ex) => `
                <li>
                  <button data-exercise="${ex.id}" class="exercise-chip tap-feedback w-full text-left rounded-btn px-3 py-2 min-h-[44px] text-body ${
                    state.selectedExerciseId === ex.id ? 'bg-accent text-base' : 'bg-base text-ink'
                  }">
                    ${escapeHtml(ex.name)}
                  </button>
                </li>
...
function renderSetForm(exerciseName, lastSet) {
  ...
  return `
    <form id="set-form" class="flex flex-col gap-3 bg-surface rounded-card p-4">
      <span class="text-card-title">${escapeHtml(exerciseName)}</span>
      <div class="flex gap-3">
        <div class="flex-1 flex flex-col gap-1">
          <label class="text-label text-muted" for="set-weight">Gewicht (kg)</label>
          <input id="set-weight" name="weight" type="number" inputmode="decimal" step="0.5" min="0" value="${weight}" class="w-full bg-base rounded-btn px-3 py-3 text-ink text-lg min-h-[44px]" required />
        </div>
        <div class="flex-1 flex flex-col gap-1">
          <label class="text-label text-muted" for="set-reps">Wiederholungen</label>
          <input id="set-reps" name="reps" type="number" inputmode="numeric" step="1" min="0" value="${reps}" class="w-full bg-base rounded-btn px-3 py-3 text-ink text-lg min-h-[44px]" required />
        </div>
      </div>
      <button type="submit" class="tap-feedback bg-accent text-base font-bold rounded-btn py-3 text-lg min-h-[44px]">
        Satz speichern
      </button>
    </form>
  `;
